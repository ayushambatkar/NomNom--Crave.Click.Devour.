...
- POST /auth/resend-otp { phoneNumber }
- POST /auth/verify-otp { phoneNumber, otp } -> { access_token, refresh_token }
- POST /auth/guest -> { access_token, refresh_token }
...
Cart (requires Bearer access token)
- GET /cart
- POST /cart/add { menuItemId, quantity }
- POST /cart/clear
- DELETE /cart/item/:menuItemId

Users
- GET /users/me
- PUT /users/me { phoneNumber?, address? }
