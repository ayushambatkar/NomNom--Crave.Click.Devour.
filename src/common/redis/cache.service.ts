...
export class CacheService {
   ...
   */
  async getRestaurant<T>(id: string, fetcher: () => Promise<T>): Promise<T> {
    return this.getOrSet(this.keys.restaurant(id), fetcher, this.TTL.RESTAURANT);
  }

  /**
   * Cache menu items for a restaurant
   */
  async getRestaurantMenu<T>(restaurantId: string, fetcher: () => Promise<T>): Promise<T> {
   ...
   */
  async getAllRestaurants<T>(fetcher: () => Promise<T>): Promise<T> {
    return this.getOrSet(this.keys.restaurantsAll(), fetcher, this.TTL.RESTAURANTS_ALL);
  }
   ...
   */
  async onRestaurantUpdated(restaurantId: string): Promise<void> {
    await Promise.all([
      this.invalidate(this.keys.restaurant(restaurantId)),
      this.invalidate(this.keys.restaurantMenu(restaurantId)),
   ...
   */
  async onMenuUpdated(restaurantId: string): Promise<void> {
    await this.invalidate(this.keys.restaurantMenu(restaurantId));
  }
   ...
   */
  async getUserOrders<T>(userId: string, fetcher: () => Promise<T>): Promise<T> {
    return this.getOrSet(this.keys.userOrders(userId), fetcher, this.TTL.USER_ORDERS);
  }

  /**
   * Invalidate user orders cache when order is created/updated
   */
  async onOrderChanged(userId: string): Promise<void> {
    await this.invalidate(this.keys.userOrders(userId));
  }
