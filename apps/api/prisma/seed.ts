...
async function createRestaurants(count = 18, ownerIds: string[]) {
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    // Assign owner cyclically from ownerIds
...
async function createRestaurantOwners(count = 5) {
  const created: any[] = [];
  for (let i = 0; i < count; i++) {
    const addr = randomPointWithinKm(CENTER_LAT, CENTER_LNG, Math.random() * MAX_RADIUS_KM);
    const p2= prisma;
    const owner = await p2.user.create({
      data: {
        phoneNumber: `+919988${(100000 + i).toString()}`,
        name: `Restaurant Owner ${i + 1}`,
        role: UserRole.RESTAURANT_OWNER,
        address: {
          create: {
            line1: `${randomInt(1, 300)} Business District`,
            city: 'Bengaluru',
            landmark: 'Near Mall',
            latitude: addr.latitude,
            longitude: addr.longitude,
...
async function main() {
  ...
  const owners = await createRestaurantOwners(5);
  const ownerIds = owners.map(o => o.id);
  console.log(`Created ${owners.length} restaurant owners`);

  // Create restaurants with owners
  const restaurants = await createRestaurants(18, ownerIds);
